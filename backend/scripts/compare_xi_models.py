"""
Multi-model XI scoring comparison.

Compares classifier, ranker, and a trivial top-by-minutes baseline on the
XI scoring task using the same rolling-origin CV as the deployed pipeline.
Each held-out (team, fixture) cell is scored by Jaccard@11 (vs actual
starters under a 4-3-3 Hungarian assignment) and NDCG@11, then aggregated
per team and league-wide into a composite rank-sum to pick a winner.
...
Run:
  cd backend
  python -X utf8 scripts/compare_xi_models.py
"""
...
# CV harness uses identical data prep, ensuring the comparison is
...
def print_report(results: dict) -> None:
    ...
    print()
    print("=" * 80)
    print("PER-MODEL AGGREGATES (sorted by Jaccard mean)")
    print("=" * 80)
    print(agg.sort_values("jaccard_mean", ascending=False).to_string(index=False))

    print()
    print("=" * 80)
    print("PER-MODEL TEAM-LEVEL AGGREGATES (mean and std of per-team Jaccard)")
    print("=" * 80)
    print(team_agg.to_string(index=False))

    print()
    print("=" * 80)
    print("COMPOSITE RANKING (rank-sum: jaccard_mean DESC, ndcg_mean DESC, "
    ...
    print()
    print(f"Composite winner: {winner}")
